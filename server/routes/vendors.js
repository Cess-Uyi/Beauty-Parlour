...
router.post("/signup",
  [
    ...
    body('vendor.businessName')
    .isString(),
    body('vendor.businessCategory')
    .isIn(['salon', 'spa', 'makeup', 'nails'])
    .withMessage('Business Category must be either salon, spa, makeup or nails'),
    body('userType')
...
router.post('/upload-photos', checkAuth, upload.array('vendorPhotos', 12), VendorsController.vendors_upload_photos)

router.get("/pending", checkAuth, VendorsController.vendors_get_pending);

router.post("/pending/finish/:appId", checkAuth, VendorsController.vendors_finish_appointment);
